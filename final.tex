...
\todo{Motivar la necesidad de estas definiciones.}
...
\todo{pasar todo el \textit{abstract nonesense} posible a esta sección}

\begin{proposition}
\label{lema-adj-cuadrados}\todo{Escribir este lema, es el Lemma 4.1.3 en el libro Riehl}
\end{proposition}
...
\todo{Agregar un ejemplo más de conjunto simplicial que no sea un complejo simplicial y represente algo geométrico, mirar algo de $\Delta$-sets en \cite{F}}
...
\begin{definition} \todo{definir esqueleto}
\end{definition}

\subsection{Productos, Coproductos, Pushouts}

\todo{definir y caracterizar los (co)productos y pushouts en $\cat{sSet}$, los uso después}

\begin{proposition}\label{sk-pushout} Si $X$ es un conjunto simplicial y $n \in \N$, entonces el diagrama
\begin{center}
\begin{tikzcd}
...
\todo[inline]{Decidir si esta demo se queda}
...
De la caracterización de $\dot{\Delta_x}$ se sigue que $\tint{\Delta_x} = \{ [(x,p)] : p \in \tint{|\ss{n}|}\}$ y por lo tanto cada función $f_x$ es biyectiva, continua, y la restricción $f_x : \partial|\ss{n}| \to \dot{\Delta_x}$ es sobreyectiva. Para concluir que se tiene una estructura celular en $|X|$ debemos ver entonces que $f_x$ es abierta al restringirla al interior de $\Delta_x$.  \todo[inline]{FALTA TERMINAR (Y REVISAR)}
\end{proof}
\end{tcolorbox}
...
\end{tikzcd}
\end{center}
existe una flecha $\ss{n} \to X$ que hace conmutar el diagrama. En ocasiones diremos que $f$ es una fibración para referirnos a que es una fibración de Kan\todo{escribir esto mejor}.
\end{definition}
...
\end{proof}
\todo{Una obs sobre la "necesidad" de trabajar con complejos de Kan}
\subsection{Ejemplos}
...
\end{proposition}
\begin{proof} Ver \cite{GJ}, Lemma 3.5. \todo{Revisar si esto se puede contar.}
\end{proof}

\begin{remark} Un \textbf{no}-ejemplo de complejo de Kan corresponde a los símplices estándar vistos como conjuntos simpliciales. \todo{desarrollar}
\end{remark}
...
\end{proposition}
\begin{proof}\todo{chequear si esto es cierto y probarlo}
\end{proof}
...
\begin{center}
\begin{tikzcd}
\horn{2}{0} \arrow{r}{(-,w,z)}\arrow[hook]{d} &X \\
